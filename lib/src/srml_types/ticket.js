...
class Ticket extends codec_1.Struct.with({
    ...
    static async create(channel, amount, challenge, privKey, pubKey) {
        const { secret } = await channel.hoprPolkadot.api.query.hopr.state(channel.counterparty);
        const winProb = types_1.createTypeUnsafe(channel.hoprPolkadot.api.registry, 'Hash', [
            new bn_js_1.default(new Uint8Array(base_1.Hash.SIZE).fill(0xff)).div(WIN_PROB).toArray('le', base_1.Hash.SIZE)
        ]);
        const channelId = await channel.channelId;
        const ticket = types_1.createTypeUnsafe(channel.hoprPolkadot.api.registry, 'Ticket', [
            {
                channelId,
        ...
        const signature = await utils_1.sign(ticket.hash, privKey, pubKey);
    ...
    static async verify(channel, signedTicket) {
        ...
        return utils_1.verify(signedTicket.ticket.hash, signedTicket.signature, channel.offChainCounterparty);
    }
