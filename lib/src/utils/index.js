 ...
 * @param counterparty AccountId of the counterparty
 */
async function getId(self, counterparty, api) {
    const registry = new types_1.TypeRegistry();
...
function waitUntil(api, forWhat, until, maxBlocks) {
    return new Promise(async (resolve, reject) => {
        ...
        let i = 0;
        const unsub = await api.query.timestamp.now(async (timestamp) => {
...
async function sign(msg, privKey, pubKey) {
    ...
    const signature = secp256k1_1.default.ecdsaSign(keyPair.publicKey, privKey);
...
async function verify(msg, signature, pubKey) {
    ...
    if (!secp256k1_1.default
        ...
        console.log(`is`, (await pubKeyToAccountId(secp256k1_1.default.ecdsaRecover(signature.sr25519PublicKey, signature.secp256k1Signature, signature.secp256k1Recovery[0]))).toU8a(), `but should be`, pubKey);
